...
namespace LearnEnglishWebApp.Data
{
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // Уникальные ключи для контента
            modelBuilder.Entity<GrammarTopic>().HasIndex(gt => gt.ContentKey).IsUnique();
            modelBuilder.Entity<GrammarTest>().HasIndex(gt => gt.TestKey).IsUnique();
            modelBuilder.Entity<VocabLesson>().HasIndex(vl => vl.ContentKey).IsUnique();
            modelBuilder.Entity<VocabTest>().HasIndex(vt => vt.TestKey).IsUnique();
            ...
            // Настройка JSONB полей (только там, где действительно нужно)
            modelBuilder.Entity<VocabLesson>()
                .Property(vl => vl.VocabularyWordIds)
                .HasColumnType("jsonb");

            modelBuilder.Entity<TestResult>()
                .Property(tr => tr.AnswerDetails)
                .HasColumnType("jsonb");
        }
            ...
            if (!context.GrammarTopics.Any())
            {
                context.GrammarTopics.AddRange(
                    new GrammarTopic
                    {
                        ...
                        ContentKey = "a1-present-simple-theory",
                        OrderIndex = 1
                    },
                    new GrammarTopic
                    {
                        ...
                        ContentKey = "a1-past-simple-theory",
                        OrderIndex = 2
                    },
                    new GrammarTopic
                    {
                        ...
                        ContentKey = "a1-future-simple-theory",
                        OrderIndex = 3
                    }
                );
                context.SaveChanges();
                ...
                if (presentSimpleTopic != null)
                {
                    context.GrammarTests.AddRange(
                        new GrammarTest
                        {
                            ...
                            Level = "A1",
                            TestKey = "a1-present-simple-test-1",
                            QuestionCount = 10,
                            PassingScore = 70,
                            TimeLimitMinutes = 15,
                            OrderIndex = 1
                        ...
                        new GrammarTest
                        {
                            ...
                            Level = "A1",
                            TestKey = "a1-present-simple-test-2",
                            QuestionCount = 15,
                            PassingScore = 80,
                            TimeLimitMinutes = 20,
                            OrderIndex = 2
